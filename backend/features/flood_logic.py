...
def build_dynamic_flood_station(loc_dict: dict, weather_dict: dict) -> dict:
    """
    Build a dynamic prototype flood risk station object for any searched location in India,
    using live weather inputs and the deterministic hydrological model.
    """
    rainfall = float(weather_dict.get("rainfall_24h", weather_dict.get("rainfall_mm", 0.0)))
    moisture = float(weather_dict.get("soil_moisture", 50.0))
    slope = float(loc_dict.get("slope_deg", 3.5))

    score = calculate_flood_risk_score(rainfall, moisture, slope)
    level = calculate_flood_risk_level(score)
    runoff = calculate_surface_runoff_index(rainfall, moisture)
    assessment = generate_flood_assessment(level, rainfall, moisture, runoff, loc_dict.get("name", "Searched Location"))

    safe_name = str(loc_dict.get("name", "location")).lower().replace(" ", "-").replace(",", "")
    safe_lat = str(loc_dict.get("latitude", 0)).replace(".", "")[:4]
    safe_lon = str(loc_dict.get("longitude", 0)).replace(".", "")[:4]
    dynamic_id = f"dynamic-{safe_name}-{safe_lat}-{safe_lon}"

    return {
        "id": dynamic_id,
        "station_code": f"DYN-{safe_name[:3].upper()}",
        "name": loc_dict.get("name", "Searched Location"),
        "state": loc_dict.get("state", "India"),
        "country": loc_dict.get("country", "India"),
        "latitude": loc_dict["latitude"],
        "longitude": loc_dict["longitude"],
        "elevation": loc_dict.get("elevation", 15.0),
        "rainfall_mm": rainfall,
        "rainfall_24h": rainfall,
        "soil_moisture": moisture,
        "soil_moisture_source": weather_dict.get("soil_moisture_source", "Volumetric Model Input"),
        "slope_deg": slope,
        "flood_risk_score": score,
        "risk_score": score,
        "flood_risk_level": level,
        ...
        "runoff_index": runoff,
        "flood_assessment": assessment,
        "seven_day_trend": weather_dict.get("seven_day_trend", [0.0] * 7),
        "last_updated": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
        "data_source": weather_dict.get("data_source", "Open-Meteo Public Telemetry (No API Key)"),
        "assessment_type": "Prototype Flood Risk Assessment",
        "is_prototype": True,
        "is_dynamic": True,
