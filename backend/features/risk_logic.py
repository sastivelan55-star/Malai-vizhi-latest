...
def classify_risk_score(score: float) -> str:
    """
    Canonical risk tier classification from a numeric risk score (0-100).

    This is the SINGLE SOURCE OF TRUTH for risk level assignment.
    All API responses and frontend displays must agree with these thresholds.
    ...
    Examples:
      ...
      39.99 → LOW
      40   → MODERATE
      49   → MODERATE   ← the buggy case this fixes
      69   → MODERATE
      70   → HIGH
      100  → HIGH
    """
...
def calculate_risk_score(rainfall_mm: float, soil_moisture: float, slope_deg: float = 32.0) -> int:
    ...
    score = round(rain_pts + moist_pts + slope_pts)
